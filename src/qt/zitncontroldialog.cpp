...
#include "ui_zitncontroldialog.h"
...
std::set<std::string> ZITNControlDialog::setSelectedMints;
std::set<CMintMeta> ZITNControlDialog::setMints;

ZITNControlDialog::ZITNControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZITNControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZITNControlDialog::~ZITNControlDialog()
{
    delete ui;
}

void ZITNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZITNControlDialog::updateList()
{
...
void ZITNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZITNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZITN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZITNControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZITNControlDialog::GetSelectedMints()
{
...
void ZITNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
