...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::ITN, nValue);
...
{
    std::string genesisblockhash = "0000077899d5ea442659925a2a8da94c8f8ef5eaf8d97bb31b38679ee2418f01";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (itn.conf).");
            QMessageBox::warning(this, "ITN Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
