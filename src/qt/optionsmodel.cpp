...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ITN);
    ...
    if (!settings.contains("nAnonymizeITNAmount"))
        settings.setValue("nAnonymizeITNAmount", 1000);

    nAnonymizeITNAmount = settings.value("nAnonymizeITNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeITNAmount"))
        SoftSetArg("-anonymizeitnamount", settings.value("nAnonymizeITNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in itn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeITNAmount:
            return QVariant(nAnonymizeITNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeITNAmount:
            nAnonymizeITNAmount = value.toInt();
            settings.setValue("nAnonymizeITNAmount", nAnonymizeITNAmount);
            emit anonymizeITNAmountChanged(nAnonymizeITNAmount);
            break;
        case CoinControlFeatures:
