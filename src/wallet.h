...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT30000IFMN = 3,
    ONLY_NONDENOMINATED_NOT30000IFMN = 4, // ONLY_NONDENOMINATED and not 30000 ITN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zITN send
enum ZerocoinSpendStatus {
    ZITN_SPEND_OKAY = 0,                            // No error
    ZITN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZITN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZITN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZITN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZITN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZITN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZITN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZITN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZITN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZITN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZITN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZITN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZITN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZITN_SPENT_USED_ZITN = 14,                      // Coin has already been spend
    ZITN_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZITN_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZITNBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZITNOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzitnAuto) const;
    ...
    */
    ...
    CzITNWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzITNTracker> zitnTracker;
    ...
    void setZWallet(CzITNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zitnTracker = std::unique_ptr<CzITNTracker>(new CzITNTracker(strWalletFile));
    }

    CzITNWallet* getZWallet() { return zwalletMain; }
    ...
    void setZITNAutoBackups(bool fEnabled)
    {
