...
public:
    ...
    bool WriteZITNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZITNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZITNSeed_deprecated(uint256& seed);
    bool EraseZITNSeed();
    bool EraseZITNSeed_deprecated();

    bool WriteZITNCount(const uint32_t& nCount);
    bool ReadZITNCount(uint32_t& nCount);
