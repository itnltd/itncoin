...
#ifndef ITN_ZITNWALLET_H
#define ITN_ZITNWALLET_H
...
public:
    CzITNWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZITN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZITN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ITN_ZITNWALLET_H
