...
CzITNWallet::CzITNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zitn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZITNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZITNSeed_deprecated()) {
                    LogPrintf("%s: Updated zITN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zitn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zitn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzITNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZITNCount(nCountLastUsed);
    else if (!walletdb.ReadZITNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzITNWallet::Lock()
{
    seedMaster = 0;
}

void CzITNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzITNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZITN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzITNWallet::LoadMintPoolFromDB()
{
...
void CzITNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzITNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzITNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zitnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzITNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZITN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zitnTracker which also adds to database
    pwalletMain->zitnTracker->Add(dMint, true);
...
void CzITNWallet::SeedToZITN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzITNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzITNWallet::GenerateDeterministicZITN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzITNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzITNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
