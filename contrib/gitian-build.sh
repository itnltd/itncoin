...
VERSION=
commit=false
url=https://github.com/itnproject/itn
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the itn, gitian-builder, gitian.sigs, and itn-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/itnproject/itn
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit itn=${COMMIT} --url itn=${url} ../itn/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../itn/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/itn-*.tar.gz build/out/src/itn-*.tar.gz ../itn-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit itn=${COMMIT} --url itn=${url} ../itn/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/itn-*.zip build/out/itn-*.exe ../itn-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit itn=${COMMIT} --url itn=${url} ../itn/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/itn-*.tar.gz build/out/itn-*.dmg ../itn-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit itn=${COMMIT} --url itn=${url} ../itn/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../itn/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/itn-*.tar.gz build/out/src/itn-*.tar.gz ../itn-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../itn/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/itn-*win64-setup.exe ../itn-binaries/${VERSION}
	    mv build/out/itn-*win32-setup.exe ../itn-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../itn/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/itn-osx-signed.dmg ../itn-binaries/${VERSION}/itn-${VERSION}-osx.dmg
	fi
	popd || exit
